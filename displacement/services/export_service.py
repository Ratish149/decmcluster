...
def generate_displacement_csv(queryset, requested_columns=None):
    """
    Generates a CSV response containing displacement data.
    Only includes columns specified in requested_columns if they are valid.
    """
    model_fields = [f.name for f in Displacement._meta.fields]
    ...
    response["Content-Disposition"] = 'attachment; filename="displacements.csv"'
    ...
    for item in queryset.only(*columns):
        row = []
        for col in columns:
