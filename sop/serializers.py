...
class SOPCommentSerializer(serializers.ModelSerializer):
...
class SOPSerializer(serializers.ModelSerializer):
    ...
    comments = SOPCommentSerializer(many=True, read_only=True)
    ...
    def validate(self, attrs):
        ...
        if status == SOP.StatusChoices.RETURNED:
            has_existing = (
            ...
            if not comment and not has_existing:
                raise serializers.ValidationError({
                    "comment": "Comment is required when returning a SOP."
                })
        return attrs
